...
# Função para garantir que o arquivo não está em uso
def wait_for_file_unlock(file_path, max_attempts=10):
    for attempt in range(max_attempts):
        try:
            ...
            print(f"Arquivo em uso, tentativa {attempt + 1}/{max_attempts}...")
            time.sleep(1)
    return False

file_path = 'imobiliaria.db'

# Remover arquivo antigo se existir
if os.path.exists(file_path):
    print("Verificando se o arquivo pode ser removido...")
    if wait_for_file_unlock(file_path):
        try:
            ...
            # Tentar continuar mesmo se não conseguir remover
    else:
        print("⚠️  Arquivo está em uso, tentando continuar...")
...
-- Inserir dados fornecidos - usando INSERT OR IGNORE para evitar erros de duplicata
...
INSERT OR IGNORE INTO Endereco (id_endereco, logradouro, numero, bairro, complemento, cidade, estado, cep) VALUES 
...
INSERT OR IGNORE INTO Visita (id_visita, data_visita, hora_visita, status_visita, fk_id_usuario, fk_id_corretor, fk_id_imovel) VALUES 
...
INSERT OR IGNORE INTO Contrato_Aluguel (id_contrato_alug, tipo, data_inicio, data_fim, valor_mensalidade, fk_id_usuario, fk_id_imovel) VALUES 
...
INSERT OR IGNORE INTO Contrato_Venda (id_contrato_venda, tipo_venda, data_inicio, data_fim, valor_negociado, fk_id_usuario, fk_id_imovel) VALUES 
...
try:
    ...
    visitas = cursor.execute("SELECT COUNT(*) FROM Visita").fetchone()[0]
    ...
    print(f"📅 Visitas inseridas: {visitas}")
    
    # Mostrar alguns dados de exemplo
    print("\n📋 Exemplo de usuários:")
    for row in cursor.execute("SELECT id_usuario, nome, login_usu FROM Usuario LIMIT 5"):
        print(f"  {row[0]}: {row[1]} ({row[2]})")
    
    print("\n📋 Exemplo de imóveis:")
    for row in cursor.execute("SELECT id_imovel, tipo, valor FROM Imovel LIMIT 5"):
