...
from ..models.models import Corretor
from ..schemas.schemas import Corretor as CorretorSchema, CorretorCreate
...
@router.get("/", summary="Listar corretores")
def read_corretores(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    try:
        corretores = db.query(Corretor).offset(skip).limit(limit).all()
        return [{"id_corretor": c.id_corretor, 
                "creci": c.creci, 
                "fk_usuario_id": c.fk_usuario_id} for c in corretores]
    except Exception as e:
